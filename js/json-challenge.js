...
/*
* Write code that calculates and then outputs the following information:
*
* Total number of users
* Number of active users
* Number of inactive users
* Grand totalUsers of balances for all users
...
let lowestBalUser = null;
let highestBalance = Number.NEGATIVE_INFINITY;
let highestBalUser = null;
...
json.forEach((user) => {
    ...
    grandTotal += removeSpecialChars(user.balance);
    averageBalArr.push(removeSpecialChars(user.balance));
    if (removeSpecialChars(user.balance) < lowestBalance) {
        lowestBalance = removeSpecialChars(user.balance);
        lowestBalUser = user.name;
    }
    if (removeSpecialChars(user.balance) > highestBalance) {
        highestBalance = removeSpecialChars(user.balance);
        highestBalUser = user.name;
    }
});

averageBalance = (averageBalArr.reduce((x, y) => {
    return x + y;
}) / averageBalArr.length).toFixed(2);
...
console.log("Number of inactive users " + inactiveUsers);
console.log("Grand totalUsers of balances for all users " + grandTotal);
console.log("Average balance per user " + averageBalance);
console.log("User with the lowest balance " + lowestBalUser);
console.log("User with the highest balance " + highestBalUser);
